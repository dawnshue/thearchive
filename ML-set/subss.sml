(* A set containing functionality to test subsets *)
...
(*check fold*)
	...
	val subsetsum: (set -> bool) -> set -> set
...
(*check fold*)
	...
	fun sub2 pred (ls, []) = ls
	|   sub2 pred (ls, h::ls2) =
			if sub pred (insert(h, ls), ls2)
			then sub2 pred (insert(h, ls), ls2)
			else sub2 pred (ls, ls2)
	...
	fun subsetsum pred [] = []
	|   subsetsum pred (l::ls) =
			if sub pred (insert(l, []), ls)
			then
			sub2 pred (insert(l, []), ls)
			else
			subsetsum pred ls
